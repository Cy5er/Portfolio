//Implementation file for our three_hit monster class
...
three_hit::three_hit(std::string name, size_t hp){ // regular constructor
    name_ = name;
    alive_ = true;
    isHit = false;
    hitpoints_ = 3;
    ...
    body.setSize(sf::Vector2f(15,10));
    i = 0;
    count = 0;
...
three_hit::three_hit(std::string name, size_t hp, sf::Vector2f position){ // constructor with specific position
    alive_ = true;
    isHit = false;
    value = 2;
    hit_player = false;
...
void three_hit::render(sf::RenderWindow &window){ // render the monster
...
void three_hit::update(std::vector<float> xx , std::vector<float> yy, int count){  // update the state of the monster
...
void three_hit::get(sf::Vector2i map[50][50], int x, int y){ // initializes the path tiles that the monster has to visit
...
void three_hit::offset(){ // initializes all the offsets that the monster needs to move by
    sf::Vector2f npos = pos_;
                ...
                while(abs(getTo_[i].first - npos.x) != 0  || abs(getTo_[i].second - npos.y) != 0 ){ // while the current coordinates are not the same as destination coordinates
                        xoff = ((float)getTo_[i].first - npos.x) * (1.f/60.f) * 9; // calculate the x offset and make the movement look smooth
                        xx.push_back(xoff); // add the x offset to the vector
                        yoff = ((float)getTo_[i].second - npos.y) * (1.f/60.f) * 9; // calculate the x offset and make the movement look smooth
