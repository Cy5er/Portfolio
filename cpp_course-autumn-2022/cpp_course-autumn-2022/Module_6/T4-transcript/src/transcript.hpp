...
class TranscriptRegistry{
    public:
    std::vector<Transcript> registry_;
    TranscriptRegistry();
    void Add(const Transcript&);
    void RemoveById(const std::string&);
    std::vector<Transcript>::const_iterator FindTranscript(const std::string&) const ;
    std::vector<std::pair<std::__cxx11::basic_string<char>, long unsigned int>> FindCourseResults(const std::string&) const;
    const std::vector<Transcript>& GetTranscripts()const;
    bool isIn(const std::string& ) const;
    size_t grr(const std::string& , std::list<std::pair<std::string, long unsigned int>> )const ;
    const std::vector<Transcript> &test = registry_; 
    bool in(const std::string&, std::list<std::pair<std::string, long unsigned int>> )const ;
