...
class DiagonalPrinter: public StringPrinter {
    public:
        DiagonalPrinter(const std::string& x = "", const std::string& y = "", std::ostream& os = std::cout)
            : StringPrinter(os), x_(x), y_(y) {}
        ~DiagonalPrinter(){}
        DiagonalPrinter* Clone() const;
        virtual StringPrinter& operator()(const std::string&);
    private:
        std::string x_;
        std::string y_;
